...
`;
  ...
  if (commandName === 'stats') {
    ...
    const menu = new StringSelectMenuBuilder()
      ...
      .setMaxValues(1)
      .addOptions(options);
    const msg = await interaction.reply({ embeds: [embed], ephemeral: false,  components: [{ type: 1, components: [menu] }] });
    const filter = (msg) => interaction.user.id === msg.user.id; 
    const collector = msg.createMessageComponentCollector({ filter, max: 1, errors: ['time'], time: 120000 });
